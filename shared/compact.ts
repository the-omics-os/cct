...
export function validateCompactArgs(value: unknown): { ok: true; action: CompactActionDefinition; args: Record<string, unknown> } | { ok: false; error: string } {
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    return { ok: false, error: "cct arguments must be a JSON object." };
  }
  const args = value as Record<string, unknown>;
  if (!Object.prototype.hasOwnProperty.call(args, "action")) return { ok: false, error: "cct requires field: action." };
  if (typeof args.action !== "string") return { ok: false, error: 'cct field "action" must be string.' };
  const action = ACTION_LOOKUP.get(args.action);
  if (!action) return { ok: false, error: `cct action "${args.action}" is invalid. Allowed actions: ${COMPACT_ACTION_NAMES.join(", ")}.` };

  const keys = Object.keys(args).filter((key) => key !== "action");
  const unknown = keys.filter((key) => !(key in GLOBAL_FIELD_TYPES)).sort();
  if (unknown.length) return { ok: false, error: `cct received unknown field(s): ${unknown.join(", ")}.` };
  const allowed = new Set([...action.required, ...action.optional]);
  const irrelevant = keys.filter((key) => !allowed.has(key)).sort();
  if (irrelevant.length) return { ok: false, error: `cct action "${action.action}" does not accept field(s): ${irrelevant.join(", ")}.` };
  const missing = action.required.filter((field) => !Object.prototype.hasOwnProperty.call(args, field)).sort();
  if (missing.length) return { ok: false, error: `cct action "${action.action}" requires field(s): ${missing.join(", ")}.` };
  ...
  for (const field of [...action.required, ...action.optional]) {
    ...
    if (type === "vote") {
      if (args[field] !== "yes" && args[field] !== "no") return { ok: false, error: 'cct field "vote" must be "yes" or "no".' };
    } else if (type === "number") {
      if (field === "minutes") {
        const minutes = args[field];
        if (typeof minutes !== "number" || !Number.isFinite(minutes) || minutes <= 0 || minutes > 120) {
          return { ok: false, error: 'cct field "minutes" must be a finite number greater than 0 and at most 120.' };
        }
      } else if (typeof args[field] !== "number") {
        return { ok: false, error: `cct field "${field}" must be number.` };
      }
    } else if (typeof args[field] !== type) {
      return { ok: false, error: `cct field "${field}" must be ${type}.` };
    }
  }
